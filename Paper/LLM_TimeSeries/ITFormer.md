...
1. **Time Token Position Encoding (TPE)** 为时序token注入**层次化位置信息**：
    ...
    - Segment（分段/周期，RoPE） 公式：HT=ΦT(T)+Ptime+Pchannel+Psegment H_T = \Phi_T(T) + P_{time} + P_{channel} + P_{segment} HT​=ΦT​(T)+Ptime​+Pchannel​+Psegment​ **作用**：解决多通道、多周期、高维时序的结构化表示问题。
2. **Learnable Instruct Tokens (LIT)** 在查询文本前预置一组**可学习指令token** I={i1,…,in} I = \{i_1, \dots, i_n\} I={i1​,…,in​}，经过自注意力后提取任务特定指导 I∗ I^* I∗。 **作用**：像“系统提示词”一样，动态告诉模型当前任务（理解/诊断/决策），提升任务适应性。
3. **Instruct Time Attention (ITA)**（两阶段注意力融合）
    - 第一阶段：通道级聚合（Channel Instruct Fusing）。
    - 第二阶段：时间-指令交叉注意力（Time Instruct Attention），用 I∗ I^* I∗ 引导时序特征融合。 **作用**：高效对齐并融合时序特征与文本查询，形成统一语义空间。
4. **Time Token as Language (TAL)** 把融合后的时序特征 Hfusion H_{fusion} Hfusion​ **直接当作语言token**，替换查询中的占位符，喂给冻结LLM生成答案。 **作用**：最小开销地把时序嵌入LLM输入，实现端到端生成。
