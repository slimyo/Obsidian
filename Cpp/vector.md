...
### 关键问题：`vector` vs `deque`

你的代码用 `vector` 实现单调队列，而参考代码用 `deque`。问题出在 `pop` 操作：

cpp

```cpp
// 你的代码
q.erase(q.begin());        // O(n) — 需要把后面所有元素往前挪一位

// 参考代码  
que.pop_front();           // O(1) — deque 头尾操作都是常数时间
```

`vector` 的内存是连续的，删除头元素必须把后面所有元素整体前移。假设当前单调队列里有 m 个元素，那这一次 `pop` 就是 O(m)。在最坏情况（比如数组单调递减时，窗口里所有元素都会进队列），单次 pop 接近 O(k)，整体就从 O(n) 退化成了 O(n·k)。

这就是 `deque` 存在的意义 —— 它专门为两端高效插入/删除设计的。**单调队列必须用 `deque`**（或者用数组+双指针手动模拟环形队列），这几乎是个固定搭配。